...
@Component({
  selector: 'app-track-customer-by-product',
  templateUrl: './track-customer-by-product.component.html',
  styleUrls: ['./track-customer-by-product.component.scss']
})

export class TrackCustomerByProductComponent implements OnInit, AfterViewInit, OnDestroy, AfterViewChecked {
  ...
  public fileName = 'Theo dõi sản phẩm theo khách hàng';
  ...
  onInitGrid() {
    ...
    this.detailCellRendererParams = {
      ...
      template: function (params: any) {
        let personName = params.data.purchaseDate;
  ...
  ngOnInit(): void {
    ...
    if (filterDate) {
      ...
      this.query.productId = JSON.parse(filterDate).productId || null;
    } else {
      this.query.startDate = new Date(new Date().getFullYear(), new Date().getMonth(), 1);
      this.query.endDate = new Date();
      this.query.productId = 0;
    }
    this.screenWidth = window.innerWidth;
    ...
    this.getListBranch();
    // this.$messageService.add({  severity: 'warn', summary: 'Thông báo', detail: 'Xin hãy chọn sản phẩm bạn quan tâm.!'});
  }

  getListProductCategory(filter: string) {
  ...
  changeProductCategory() {
    this.query.productCategoryId = this.infoCategory ? this.infoCategory.categoryId : null;
  }
  ...
  getListproduct(filter: string) {
    const queryParams = queryString.stringify({
      retailerId: this.authService?.getRetailerId()
      , branchId: this.query.branchId, categoryId: this.query.productCategoryId, search: filter
    });
    this.$service.getListProduct(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {
  ...
  changeProduct() {
    this.query.productId = this.infoProduct.productId;
    this.query.page = 1;
    this.query.size = 20;
    this.first = 1;
    this.countRecord = {
  ...
  find() {
    if (!this.infoProduct) {
      this.$messageService.add({severity: 'warn', summary: 'Thông báo', detail: 'Bạn cần chọn sản phẩm để tìm kiếm .'});
      return;
    }
    this.getLists();
  ...
  getLists() {
    ...
    localStorage.setItem('filterDate', JSON.stringify({
      ...
      , startDate: params.startDate, productId: params.productId
    }));
    const queryParams = queryString.stringify(params);
    this.$service.getTrackCustomerByProduct(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {
  ...
  getDaitel(purchaseDate: string, event: any) {
    ...
    //         // event.api.refreshServerSide({ route: productId, purge: true })

...
@Component({
  selector: 'app-follow-debt-customer',
  templateUrl: './follow-debt-customer.component.html',
  styleUrls: ['./follow-debt-customer.component.scss']
})
export class FollowDebtCustomerComponent implements OnInit, AfterViewInit, OnDestroy, AfterViewChecked {
  ...
  public cols = [
    ...
    {field: 'debt', header: 'Công nợ', typeField: 'decimal', aggFunc: 'sum'}
  ];

  public colsDetail: any[] = [
    {field: 'invoiceNo', header: 'Số hóa đơn', typeField: 'text', headerClass: 'bg-primary-reverse', cellClass: ['bg-primary-reverse']},
    {
      field: 'purchaseDate',
      header: 'Ngày hóa đơn',
      typeField: 'text',
    ...
    {field: 'productName', header: 'Sản phẩm', typeField: 'text', headerClass: 'bg-primary-reverse', cellClass: ['bg-primary-reverse']},
    {field: 'unit', header: 'Đơn vị', typeField: 'text', headerClass: 'bg-primary-reverse', cellClass: ['bg-primary-reverse']},
    {
      field: 'quantity',
      header: 'Số lượng',
      typeField: 'decimal',
  ...
  onInitGrid() {
    ...
    this.detailCellRendererParams = {
      ...
      template: function (params: any) {
        ...
        // const total = eval(params.data.childrens.map((item: any) => item.debt).join('+'))
        return (
  ...
  getLists() {
    ...
    this.$service.getCustomerDebt(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
  ...
  getDaitel(customerId: string, event: any) {
    ...
    localStorage.setItem('filterDate', JSON.stringify({endDate: params.endDate, startDate: params.startDate}));
    const queryParams = queryString.stringify(params);
    this.$service.getCustomerDebtDetail(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {

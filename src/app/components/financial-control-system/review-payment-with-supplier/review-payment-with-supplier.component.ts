...
@Component({
  selector: 'app-review-payment-with-supplier',
  templateUrl: './review-payment-with-supplier.component.html',
  styleUrls: ['./review-payment-with-supplier.component.scss']
})

export class ReviewPaymentWithSupplierComponent implements OnInit, AfterViewInit, OnDestroy, AfterViewChecked {
  ...
  public cols: any[] = [
    ...
    {field: 'needToPayAmount', header: 'Phải trả (Trên Phần Mềm)', typeField: 'decimal'},
    {field: 'realPayAmount', header: 'Thực trả', typeField: 'decimal'},
  ...
  getLists() {
    ...
    this.$service.getReviewPaymentWithSupplier(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {

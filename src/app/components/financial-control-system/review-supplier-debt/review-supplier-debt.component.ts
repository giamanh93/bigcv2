...
@Component({
  selector: 'app-review-supplier-debt',
  templateUrl: './review-supplier-debt.component.html',
  styleUrls: ['./review-supplier-debt.component.scss']
})
export class ReviewSupplierDebtComponent implements OnInit, AfterViewInit, OnDestroy, AfterViewChecked {
  ...
  public fileName = 'Kiểm soát công nợ nhà cung cấp';
  suppliers: any[] = [];
  public query: any = {
  ...
  public cols: any[] = [
    ...
    {field: 'purchaseNo', header: 'Mã hóa đơn', typeField: 'text'},
    ...
    // {field: 'price', header: 'Đơn giá', typeField: 'decimal'},
  ...
  public getRowId: GetRowIdFunc = (params: GetRowIdParams) => {
    return params.data.purchaseOrderId ;
  };
  ...
  getListSupplier(filter: string) {
    const queryParams = queryString.stringify({
      retailerId: this.authService?.getRetailerId()
      , branchId: this.query.branchId, search: filter
    });
    this.$service.getListSupplier(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {
  ...
  changeSupplier() {
    this.query.supplierId = this.infoSupplier.supplierId;
    this.query.page = 1;
    this.query.size = 20;
    this.first = 1;
    this.countRecord = {
  ...
  getLists() {
    ...
    localStorage.setItem('filterDate', JSON.stringify({
      ...
      , startDate: params.startDate, supplierId: params.supplierId
    }));
    const queryParams = queryString.stringify(params);
    this.$service.getReviewSupplierDebt(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {
  ...
  rowGroupOpenedCallback(event: any) {
    if (event.data.childrens && event.data.childrens.length === 0) {
      const index = this.listDatas.findIndex(d => d.purchaseOrderId === event.data.purchaseOrderId);
      this.getDaitel(event.data.purchaseOrderId, event);
    }
  }

  getDaitel(purchaseOrderId: string, event: any) {
    const params = {page: this.query.page, size: this.query.size, purchaseOrderId: purchaseOrderId};
    const queryParams = queryString.stringify(params);
    this.$service.getReviewSupplierDebtDetail(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {
          ...
          event.api.forEachNodeAfterFilterAndSort((rowNode: any, index: number) => {
            ...
            if (rowNode.data.purchaseOrderId === purchaseOrderId) {

...
@Component({
  selector: 'app-review-supplier-debt-by-product',
  templateUrl: './review-supplier-debt-by-product.component.html',
  styleUrls: ['./review-supplier-debt-by-product.component.scss']
})

export class ReviewSupplierDebtByProductComponent implements OnInit, AfterViewInit, OnDestroy, AfterViewChecked {
  ...
  public cols: any[] = [
    {field: 'supplierId', header: 'Mã nhà cung cấp', typeField: 'text', masterDetail: false,},
    {field: 'supplierName', header: 'Nhà cung cấp', typeField: 'text'},
    {field: 'debtAmount', header: 'Tiền còn nợ', typeField: 'decimal'},
  ];
  public groupDisplayType: RowGroupingDisplayType = 'groupRows';
  public colsDetail: any[] = [
    {field: 'purchaseNo', header: 'Số phiếu nhập', typeField: 'text', headerClass: 'bg-primary-reverse', cellClass: ['bg-primary-reverse']},
    {
      field: 'supplierName',
      header: 'Nhà cung cấp',
      typeField: 'text',
    ...
    {
      field: 'needToPayAmount',
      header: `Phải trả (Trên Phần Mềm)`,
      typeField: 'decimal',
      aggFunc: 'sum',
      headerClass: 'bg-primary-reverse',
      cellClass: ['bg-primary-reverse']
  ...
  onInitGrid() {
    ...
    this.detailCellRendererParams = {
      ...
      template: function (params: any) {
        ...
        const total = eval(params.data.childrens.map((item: any) => item.realPayAmount).join('+'));
        return (
  ...
  getLists() {
    ...
    this.$service.getReviewSupplierDebtByProduct(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {
  ...
  find() {
    if (!this.query.productId || this.query.productId === null) {
      this.$messageService.add({severity: 'warn', summary: 'Error Message', detail: 'Sản phẩm bắt buộc chọn'});
      return;
    }
    this.getLists();
  }
  ...
  getDaitel(purchaseDate: string, event: any) {
    ...
    localStorage.setItem('filterDate', JSON.stringify({endDate: params.endDate, startDate: params.startDate}));
    const queryParams = queryString.stringify(params);
    this.$service.getReviewPaymentWithSupplierDetail(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {

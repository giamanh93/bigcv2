...
@Component({
  selector: 'app-comming-up-customer',
  templateUrl: './comming-up-customer.component.html',
  styleUrls: ['./comming-up-customer.component.scss']
})
export class CommingUpCustomerComponent implements OnInit, AfterViewInit, OnDestroy, AfterViewChecked {
  ...
  public fileName: string = 'Danh sách khách hàng đến chu kỳ mua';
  public query: any = {
  ...
  public cols: any[] = [
    {field: 'customerCode', header: 'Mã KH', typeField: 'text', masterDetail: false},
    {field: 'customerName', header: 'Tên KH', typeField: 'text', width: 200},
    {field: 'invoiceCount', header: 'Số lượng HĐ', typeField: 'decimal'},
    {field: 'averageDay', header: 'Số ngày TB', typeField: 'decimal'},
    {field: 'lastPurchaseDate', header: 'Ngày mua cuối cùng', typeField: 'text'},
  ];
  ...
  getLists() {
    ...
    this.$service.getCommingUpCustomer(queryParams)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe(results => {
        if (results.success) {
  ...
  private $service = inject(UtilityManagerService);
